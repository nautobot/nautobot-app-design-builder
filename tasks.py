...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_design_builder.ephemeral_ports
        and context.nautobot_design_builder.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_design_builder.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_design_builder.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_design_builder.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_design_builder.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_design_builder.python_ver
    dockerfile_path = os.path.join(context.nautobot_design_builder.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_design_builder.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
@task(
    help={
        "version": "Version of Nautobot Design Builder to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_design_builder'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_design_builder")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_design_builder.migrations"
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_design_builder/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Nautobot Design Builder."""
    command = "nautobot-server generate_nautobot_design_builder_test_data"
    if database:
